...
def create_labelee_foundation(config: Dict = None):
    ...
    model = LabeleeFoundation(**default_config)
...
if __name__ == "__main__":
    ...
    custom_config = {
        'vision_model_name': 'vit_large_patch16_224.augreg_in21k_ft_in1k',
        'text_model_name': 'bert-base-uncased',
        'feature_dim': 1024,
        'num_classes': 10000
    }
    # model = create_labelee_foundation(custom_config)
    
    # Initialize loss function
    criterion = MultiTaskLoss(alpha=1.0, beta=0.5, gamma=0.7, delta=0.3)
    ...
    similarity_output = model(images, input_ids, attention_mask, task='similarity')
    print(f"Similarity output shape: {similarity_output.shape}")
    
    contrastive_output = model(images, input_ids, attention_mask, task='contrastive')
    print(f"Contrastive output shapes: {[x.shape for x in contrastive_output]}")
    
    # Get all features for analysis
    features = model(images, input_ids, attention_mask, return_features=True)
